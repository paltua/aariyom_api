...
class Event_model extends CI_Model
{
    ...
    public function getEventByProgramme($program_id = 0)
    {
        $this->db->select('EM.*, EML.*');
        $this->db->from('event_master EM');
        $this->db->join('event_master_log EML', 'EML.eml_id=EM.eml_id', 'INNER');
        $this->db->join('event_programs_rel EPR', 'EPR.eml_id = EML.eml_id', 'INNER');
        $this->db->join('programs PRO', 'PRO.program_id = EPR.program_id AND PRO.is_deleted = "no"', 'INNER');
        $this->db->where('EML.event_status !=', 'deleted');
